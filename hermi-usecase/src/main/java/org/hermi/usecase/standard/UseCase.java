 ...
 * discovered Just-In-Time within {@code doFulfill(Context)}. The base class validates Context and
 ...
 * their Default implementations. WHEN: Callers invoke {@code fulfill(Context)} — it validates, then
 * delegates to {@code doFulfill}. NEVER call {@code doFulfill} directly. Subclasses override {@code
 * doFulfill}, NEVER {@code fulfill}. WHERE: Use Case layer (Phase 1) — the central unit of business
 ...
 * Discover I/O contracts JIT within {@code doFulfill}.
 *
 * <p>DO NOT add: - try-catch (infrastructure exceptions MUST be caught and wrapped as domain
 * exceptions in doFulfill) - log statements (handled by {@link org.hermi.commons.audit.Auditor}) -
 * null checks on context (handled by {@link Validatable} validation in fulfill()) - framework
 ...
 * // WRONG: Do NOT add @Service, @Transactional, or local try-catch in doFulfill
 * }</pre>
 */

/**
 * Blueprint-First Orchestration: validates Context, delegates to {@code doFulfill}, integrates
 ...
 * Subclasses implement {@code doFulfill(Context)} to define the business logic, discovering I/O
 ...
 */
public abstract class UseCase<C, R> extends Executor<C, R> implements Intent<C, R> {
  protected UseCase() {
   ...
   *
   * <p>Validates the context, delegates to {@link #doFulfill}, then validates and audits the
   * result. NEVER call {@code doFulfill} directly.
   *
   ...
   *
   * <p>The base {@code fulfill(Context)} method validates the context before calling this method.
