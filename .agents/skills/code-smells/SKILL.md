...
Smells are organized as a flat list of categories — each category IS a smell. The first 24 match Martin Fowler's *Refactoring* (2nd Edition, Chapter 3); the rest are modern extensions. To add a new smell, add a new `###` category block following the same pattern. Smell IDs use the pattern `S-{category}-{position}` (e.g., `S-01-03` = category 1, 3rd smell). Adding a smell only increments the position within its category — nothing else shifts.

---

### Mysterious Name
Names that fail to reveal intent — whether through contradiction, obscurity, negation, or just being meaningless.

#### S-01-01 — Temporary Field
```
...
#### S-01-03 — Alternative Classes with Different Interfaces
```
...
### Duplicated Code
The same code structure (logic or documentation) appearing in multiple places.

#### S-02-01 — Divergent Change
```
...
### Long Function
Functions whose bodies are too long or too deeply nested to understand at a glance.

#### S-03-01 — Contradictory Naming
```
...
### Long Parameter List
Methods that take too many arguments, or arguments that select between behaviors.

#### S-04-01 — Avoid Negative Conditionals
```
...
### Global Data
Mutable data accessible from anywhere — concurrency risk, coupling magnet, untestable.

#### S-05-01 — Incorrect Behavior at Boundaries
```
...
### Mutable Data
Data that changes in non-obvious ways — output arguments, side effects, unprotected mutation.

#### S-06-01 — Refused Bequest
```
...
### Divergent Change
One class or method that must change for unrelated reasons.

#### S-07-01 — Flag / Selector Arguments
```
...
### Shotgun Surgery
A single logical change requires touching many files.

#### S-08-01 — Inappropriate Static
```
...
### Data Clumps
The same group of fields appears together everywhere — a class waiting to be born.

#### S-10-01 — Non-Compliant Naming
```
...
### Primitive Obsession
Domain concepts represented by raw primitives or nameless literals instead of proper types.

#### S-11-01 — Duplicate Code
```
...
#### S-11-02 — Inappropriate Intimacy
```
...
### Repeated Switches
Type-based branching duplicated across the codebase instead of using polymorphism.

#### S-12-01 — Duplicate Documentation
```
...
### Loops
Fowler advocates replacing loops with first-class functions (`map`/`filter`/`reduce`). No detection rules in this skill yet — this category is a refactoring target rather than a structural smell.

*(No detection rules in this skill.)*

---

### Lazy Element
A class, method, or code block that doesn't justify its existence — dead or barely alive.

#### S-14-01 — Logging Intrusion
```
...
#### S-14-02 — Parallel Inheritance Hierarchies
```
...
Abstractions built for futures that never arrive — YAGNI violations.

#### S-15-01 — Shotgun Surgery
```
...
### Temporary Field
Fields that are only meaningful during a narrow slice of an object's lifecycle.

#### S-16-01 — Obscure Code
```
...
### Message Chains
Chains of getter calls that couple the caller to every link in the navigation path.

#### S-17-01 — Magic Literals
```
...
### Middle Man
A class whose public interface is mostly pass-through delegation.

#### S-18-01 — Switch Statements
```
...
### Insider Trading
Classes that access each other's private parts or form bidirectional tangles.
...
### Large Class
A class that has accumulated too many fields, methods, or responsibilities.

#### S-20-01 — Encapsulate Conditionals
```
...
Two classes that do the same thing but expose different method names or signatures.

#### S-21-01 — Long Parameter List
```
...
### Data Class
A class that is pure container — fields and accessors, no behavior on its own data.
...
### Refused Bequest
A subclass that rejects its inheritance by throwing, no-opping, or ignoring superclass methods.
...
### Comments
Inline explanatory comments that paper over code that should be self-documenting.

#### S-24-01 — Incomplete Library Class
```
...
### Vertical Separation
Callers and callees kept far apart, variables declared far from use — files that don't read top-to-bottom.

#### S-25-01 — Speculative Generality
```
...
Unvalidated inputs, unexpected null returns, silently swallowed exceptions — broken contracts.

#### S-26-01 — Vertical Separation
```
...
### CRITICAL
- [S-02-01] Duplicate Code — `processOrder()` in OrderService and `handleOrder()` in CartService
  FILE: src/OrderService.java:L45–L67
  FIX: Extract shared logic to `OrderProcessor.execute(order)`

### WARNING
- [S-04-01] Long Parameter List — `createUser(name, email, phone, role, deptId, isAdmin)` has 6 params
  FILE: src/UserFactory.java:L12
  FIX: Introduce `UserCreationRequest` parameter object

### INFO
- [S-15-01] Speculative Generality — `AbstractReportFormatter` has one subclass and no planned extensions
